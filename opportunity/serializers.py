...
class OpportunitySerializer(serializers.ModelSerializer):
    ...
    vio_id = serializers.IntegerField(required=True)
    ...
    def validate(self, attrs):
        if not Vio.objects.filter(id=attrs["vio_id"]).exists():
            raise serializers.ValidationError({"vio_id": "There is nO VIO with the id provided"})

        return super().validate(attrs)

    @transaction.atomic()
    def create(self, validated_data):
        vio = Vio.objects.get(id=validated_data["vio_id"])
...
class ApplyForOpportunitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('volunteer', 'vio')

    def validate(self, attrs):
        if not Volunteer.objects.filter(id=attrs["volunteer_id"]).exists():
